...
parser.add_argument('--data_split', type=int, nargs='+', default=[8,1,1], help='train, valid, test split')
# mvecf 
...
data
""" 
node_features, edge_features, full_data, train_data, val_data, test_data, upper_u = get_data(dataset_name='transaction', period=args.period, data_split=args.data_split)
      ...
      if args.model_name == 'ours': 
        ...
        for idx, (stocks_p, source, items_c, ts) in enumerate(zip(portfolios_batch, sources_batch, candidates_batch, timestamps_batch)):
          ...
          for idx, (c, feature) in enumerate(zip(items_c, cand_feature)):
            ...
            # # (3) y_tilde 계산: y_tilde vs threshold 비교 -> y_tilde 순위에 따라 샘플링 -> (x)
