...
parser.add_argument('--lambda_mv', type=float, default=0)
  ...
  for batch in tqdm(range(0, num_batch, BACKPROP_EVERY), total=num_batch//BACKPROP_EVERY, desc="Progress: Train Batch Loop"):
    ...
    for j in range(BACKPROP_EVERY):
      ...
      if args.model_name == 'ours': 
        ...
        y_mv_pos, y_tilde_pos, y_pos, cui_pos, cmv_pos = [], [], [], [], []
        y_mv_neg, y_tilde_neg, y_neg, cui_neg, cmv_neg = [], [], [], [], []
        for idx, (stocks_p, source, items_c, ts) in enumerate(zip(portfolios_batch, sources_batch, candidates_batch, timestamps_batch)):
          ...
          # (6) y_tilde_list를 sorting한 index를 이용해서 items_c를 정렬하기
          sorted_idx = np.argsort(y_tilde_list)[::-1]
          sorted_items_c = items_c[sorted_idx]
        ...
        ### inter loop done ### 
        # save lists as pkl: y_mv_pos, y_tilde_pos, y_mv_neg, y_tilde_neg
        ...
        # bsbsbs = len(sources_batch_final)

        # # reshape source and destination to (bs, 1, emb_dim) 
        # # source_embedding = source_embedding_final.view(bsbsbs, 1, -1)

        # # reshape p_pos and p_neg to (bs, k, emb_dim) 
        # p_pos_embedding = p_pos_embedding.view(bsbsbs, 1, -1)
        # p_neg_embedding = p_neg_embedding.view(bsbsbs, args.p_neg_num, -1)
